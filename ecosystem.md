...
```
                              ┌─────────────────────────────────────────────┐
                              │             CLIENTS / FRONTENDS             │
                              │  (Web app, mobile, SDK, admin dashboard)    │
                              └────────────────────┬────────────────────────┘
                                                   │
                                        HTTP / WebSocket
                                                   │
...
| `192.168.1.128` | VM, RTX 4060 8 GB (passthrough), 64 GB RAM, 4 vCPU | `kimini`, `gothmog`, `test_dbs` (PostgreSQL/Redis/MinIO/Qdrant), `sauron`, `mimiri` (`rag-ingest`) | Central control/data/observability node. Colocating `mimiri` with Qdrant reduces ingest path complexity and cross-host vector write latency. |
