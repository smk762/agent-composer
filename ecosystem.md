...
```
  ...
  │ NAS (.121)           │  │ NAS (.121)       │  │ NAS (.121)               │  │ .138 (agent-     │
  │                      │  │                  │  │                          │  │  composer)       │
...
RAG ingestion is served by `rag-ingest` on `192.168.1.138:9050` (inside `agent-composer`), colocated with Qdrant on `192.168.1.138:6333`, using Ollama container-locally at `http://ollama:11434`.
...
| `192.168.1.138` | VM, RTX 4060 8 GB (passthrough), 16 GB RAM, 8 vCPU | `agent-composer` (Ollama + Qdrant + rag-ingest + ModernBERT), `tss-stack` *(pending)*, `sauron` *(pending)* |
...
| `tss-stack` | `192.168.1.138` *(pending migration)* |
| `sauron` | `192.168.1.138` *(pending migration)* |
...
curl -fsS http://192.168.1.138:9090/-/healthy || echo "prometheus down"
curl -fsS http://192.168.1.138:9100/metrics >/dev/null || echo "node exporter down"
...
- `tss-stack` and `sauron` expected to fail until migration to `.138` is complete.
...
- `agent-composer` with local Ollama, Qdrant, and `rag-ingest` runs on `.138`.
- `tss-stack` and `sauron` will migrate to `.138` when ready.
- PostgreSQL (`:5433`), Redis (`:6380`), and MinIO (`:9000`) run on the DS923+ NAS (`.121`) — stateful infra decoupled from compute nodes.
- Qdrant is container-local inside `agent-composer` on `.138`; `rag-ingest` uses `QDRANT_URL=http://qdrant:6333`.
...
- [ ] Confirm all ingest callers/scrape targets point to `192.168.1.138:9050`.
